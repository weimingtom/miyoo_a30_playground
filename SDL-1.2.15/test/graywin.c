...
void DrawBox2(SDL_Surface *screen)
{
	static unsigned int seeded = 0;
	SDL_Rect area;
	Uint32 color;
	...
	area.w = 320;
	area.h = 240;
        area.x = 50;//200;//320-(area.w/2); //0
	area.y = 50;//100;//240-(area.h/2); //0
        ...
        {
	    //0xFFFF0000;//==SDL_MapRGB(screen->format, 0xff, 0, 0x0)//==blue not good
            //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, vid.vsurf->w, vid.vsurf->h, 0, GL_RGBA, GL_UNSIGNED_BYTE, vid.vsurf->pixels);
            //
            //color = SDL_MapRGB(screen->format, 0xff, 0, 0x0);
	    color = SDL_MapRGBA(screen->format, 0xFF, 0, 0x00, 0xFF);
        }
