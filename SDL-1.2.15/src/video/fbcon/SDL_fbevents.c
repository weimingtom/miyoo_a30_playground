...
fprintf(stderr, "FB_EnterGraphicsMode 1\n");
	/* Set medium-raw keyboard mode */
	if ( (keyboard_fd >= 0) && !FB_InGraphicsMode(this) ) {
fprintf(stderr, "FB_EnterGraphicsMode 2\n");
		/* Switch to the correct virtual terminal */
#if 0 //added 20240609
		if ( current_vt > 0 ) {
			struct vt_stat vtstate;
fprintf(stderr, "FB_EnterGraphicsMode 2.1\n");
			if ( ioctl(keyboard_fd, VT_GETSTATE, &vtstate) == 0 ) {
				saved_vt = vtstate.v_active;
			}
fprintf(stderr, "FB_EnterGraphicsMode 2.2\n");
			if ( ioctl(keyboard_fd, VT_ACTIVATE, current_vt) == 0 ) {
				ioctl(keyboard_fd, VT_WAITACTIVE, current_vt);
			}
		}
#endif
fprintf(stderr, "FB_EnterGraphicsMode 3\n");
		...
		if ( tcgetattr(keyboard_fd, &saved_kbd_termios) < 0 ) {
			...
			keyboard_fd = -1;
fprintf(stderr, "FB_EnterGraphicsMode 3.1\n");
			return(-1);
		}
fprintf(stderr, "FB_EnterGraphicsMode 4\n");
#if 0		
...
fprintf(stderr, "FB_EnterGraphicsMode 5\n");
	}
fprintf(stderr, "FB_EnterGraphicsMode 6\n");
	return(keyboard_fd);
}
