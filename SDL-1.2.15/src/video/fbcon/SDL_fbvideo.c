...
#ifdef FBCON_DEBUG
	fprintf(stderr, "FB_SetVideoMode 1\n");
#endif
...
#ifdef FBCON_DEBUG
	fprintf(stderr, "FB_SetVideoMode 2\n");
#endif
...
#ifdef FBCON_DEBUG
	fprintf(stderr, "FB_SetVideoMode 3\n");
#endif
...
static void FB_blit16(Uint8 *byte_src_pos, int src_right_delta, int src_down_delta,
		Uint8 *byte_dst_pos, int dst_linebytes, int width, int height, int vinfo_width, int vinfo_height, int src_x1, int src_y1)
{
#if !defined(USE_NONE)
	int w;
...
int x, y;
	...
	for (y = 0; y < height; ++y) {
		Uint32 *src = src_pos + y * src_down_delta; //src_down_delta = w
		Uint32 *dst = (Uint32 *)((Uint8 *)dst_pos + y * dst_linebytes);  //dst_linebytes = dst.w
		for (x = 0; x < width; ++x) {
			*(dst + x * 1) = (0xFF000000 | *(src + x * src_right_delta)); //src_right_delta=1
		}
	}
#elif 0
...
int x, y;
	...
	for (y = 0; y < height; ++y) {
		...
		//src_right_delta=1
		for (x = 0; x < width; ++x) {
			*((Uint32 *)((Uint8 *)dst_pos + y * dst_linebytes) + x * 1) = (0xFF000000 | *(src_pos + y * src_down_delta + x * src_right_delta));
		}
	}
#elif 1
printf("<<< FB_blit16: w=%d,h=%d,vw=%d,vh=%d,x1=%d,y1=%d\n", width, height, vinfo_width, vinfo_height, src_x1, src_y1);
...
int x, y;
	...
	for (y = 0; y < height; ++y) {
		...
		//src_right_delta=1
		for (x = 0; x < width; ++x) {
			*((Uint32 *)((Uint8 *)dst_pos + (vinfo_width - 1 - x - src_y1 - src_x1) * dst_linebytes) + (y - src_x1 + src_y1) * 1) = (0xFF000000 | *(src_pos + (y) * src_down_delta + x * src_right_delta));
		}
	}
#endif
	...
	while (height > 0) {
		...
		for (w = width; w > 0; w -= BLOCKSIZE_W) {
			FB_blit16((Uint8 *)src,
					...
					dst_linebytes,
					min(w, BLOCKSIZE_W),
					min(height, BLOCKSIZE_H), vinfo_width, vinfo_height, src_x1, src_y1);
...
static void FB_DirectUpdate(_THIS, int numrects, SDL_Rect *rects)
{
...
#ifdef USE_NONE
#ifdef FBCON_DEBUG
	fprintf(stderr, "<<<FB_DirectUpdate %d\n", numrects);
#endif
#endif
	...
	for (i = 0; i < numrects; i++) {
		...
		blitFunc((Uint8 *) src_start,
				...
				physlinebytes,
				scr_x2 - scr_x1,
				scr_y2 - scr_y1, width, height, scr_x1, scr_y1);
	}
}
